    ...
    module Datajud
      module Parse
        ...
        def process(body)
          ...
          {
            number_process: body.dig(:num_process),
            court: body.dig(:destin, :court),
            sub_court: body.dig(:destin, :sub_court),
            number_process_api: body&.dig(:result,'hits','hits').first&.dig("_source", "numeroProcesso"),
        ...
        def movements_process_size(body)
          body&.dig(:result, 'hits','hits').first&.dig("_source", "movimentos")&.size
        end

        def movements_process(body)
          body&.dig(:result, 'hits','hits').first&.dig("_source", "movimentos")
        end

        def subject_process(body)
          str_returned = ''

          subj = body&.dig(:result, 'hits','hits').first&.dig("_source", "assuntos")
        ...
        def classe_process(body)
          ...
          #["codigo", "nome"]
          body&.dig(:result, 'hits','hits').first&.dig("_source", "classe", "nome")
        end

        def name_system(body)
          body&.dig(:result, 'hits','hits').first&.dig("_source", "sistema", "nome")
        end

        def process_format(body)
          body&.dig(:result, 'hits','hits').first&.dig("_source", "formato", "nome")
        end

        def name_court(body)
          body&.dig(:result, 'hits','hits').first&.dig("_source", "tribunal")
        end

        def last_updated_info(body)
          convert_date_time(
            body&.dig(:result, 'hits','hits').first&.dig("_source", "dataHoraUltimaAtualizacao")
          )
        rescue
          body&.dig(:result, 'hits','hits').first&.dig("_source", "dataHoraUltimaAtualizacao")
        end
        ...
        def rate(body)
          body&.dig(:result, 'hits','hits').first&.dig("_source", "grau")
        end

        def filing_date(body)
          convert_date_time(
            body&.dig(:result, 'hits','hits').first&.dig("_source", "dataAjuizamento")
          )
        end

        def api_datajud_id(body)
          body&.dig(:result, 'hits','hits').first&.dig("_source", "id")
        end

        def level_of_reservation(body)
          body&.dig(:result, 'hits','hits').first&.dig("_source", "nivelSigilo")
        end
        ...
        def orgao_julgador(body)
          ...
          str  = "#{body&.dig(:result, 'hits','hits').first&.dig('_source', 'orgaoJulgador', 'codigo')} -"
          str << " #{body&.dig(:result, 'hits','hits').first&.dig('_source', 'orgaoJulgador', 'nome')}"
          str
        end

        def source_api(body)
          body&.dig(:result, 'hits','hits').first&.dig('_index') || ''
        end

        def name_process_api(body)
          body&.dig(:result, 'hits','hits').first&.dig("_id") || ''
        end

        def score(body)
          body&.dig(:result, 'hits','hits').first&.dig("_score") || '00.00'
        end
      end
    end
  end
